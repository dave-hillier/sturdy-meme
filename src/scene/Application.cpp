...
bool Application::init(const std::string& title, int width, int height) {
    ...
    // Start near terrain center to see roads/rivers visualization
    {
        const auto& terrain = renderer_->getSystems().terrain();
        float cameraX = -2000.0f, cameraZ = -2000.0f;  // Position toward terrain center
        float terrainY = terrain.getHeightAt(cameraX, cameraZ);
        camera.setPosition(glm::vec3(cameraX, terrainY + 50.0f, cameraZ));  // Higher to see more
        camera.setYaw(-45.0f);   // Look toward terrain center
        camera.setPitch(-20.0f); // Downward tilt to see ground
    }
