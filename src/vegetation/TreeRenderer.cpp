...
void TreeRenderer::render(vk::CommandBuffer cmd, uint32_t frameIndex, float time,
    ...
    for (const auto& renderable : branchRenderables) {
        ...
        // Read bark type from ECS when available, fall back to Renderable
        std::string barkTypeName = renderable.barkType;
        glm::mat4 model = renderable.transform;
        Mesh* mesh = renderable.mesh;

        if (ecsWorld_ && treeIdx >= 0) {
            ecs::Entity e = treeSystem.getTreeEntity(static_cast<uint32_t>(treeIdx));
            if (e != ecs::NullEntity) {
                if (auto* bark = ecsWorld_->tryGet<ecs::BarkType>(e)) {
                    barkTypeName = bark->typeName;
                }
                if (auto* xform = ecsWorld_->tryGet<ecs::Transform>(e)) {
                    model = xform->matrix;
                }
                if (auto* meshRef = ecsWorld_->tryGet<ecs::MeshRef>(e)) {
                    mesh = meshRef->mesh;
                }
            }
        }

        if (barkTypeName != lastBarkType) {
            vk::DescriptorSet descriptorSet = getBranchDescriptorSet(frameIndex, barkTypeName);
            vkCmd.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, **branchPipelineLayout_,
                                     0, descriptorSet, {});
            lastBarkType = barkTypeName;
        }
    ...
    if (renderBranches && !branchRenderables.empty()) {
        ...
        if (useInstancedPath) {
            ...
            for (const auto& renderable : branchRenderables) {
                ...
                if (barkTypeName != lastBarkType) {
                    vk::DescriptorSet branchSet = getBranchDescriptorSet(frameIndex, barkTypeName);
                    vkCmd.bindDescriptorSets(vk::PipelineBindPoint::eGraphics,
