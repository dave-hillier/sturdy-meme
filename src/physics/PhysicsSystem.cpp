...
PhysicsBodyID PhysicsWorld::createTileHeightfield(const float* samples, uint32_t sampleCount,
    ...
    // Resample height data with bilinear interpolation and convert to world space
    std::vector<float> joltSamples(joltSampleCount * joltSampleCount);
    float srcScale = static_cast<float>(sampleCount - 1) / static_cast<float>(joltSampleCount - 1);
    ...
    // The scale parameter determines the XZ spacing between samples
    float xzScale = tileSize / (joltSampleCount - 1);

    // Position the heightfield at the tile's world position
    JPH::HeightFieldShapeSettings heightFieldSettings(
        joltSamples.data(),
        JPH::Vec3(worldMinX, 0.0f, worldMinZ),  // Offset: tile position
        JPH::Vec3(xzScale, 1.0f, xzScale),       // Scale: XZ spacing, Y is direct
        joltSampleCount
    );
    ...
    JPH::ShapeSettings::ShapeResult shapeResult = heightFieldSettings.Create();
    if (!shapeResult.IsValid()) {
        SDL_Log("Failed to create tile heightfield shape: %s", shapeResult.GetError().c_str());
        return INVALID_BODY_ID;
    }
    ...
    JPH::Body* body = bodyInterface.CreateBody(bodySettings);
    if (!body) {
        SDL_Log("Failed to create tile heightfield body");
        return INVALID_BODY_ID;
    }
