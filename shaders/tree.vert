...
void main() {
    ...
    float branchLevel = vert.position.w;
    vec3 localNormal = vert.normal.xyz;
    vec3 localTangent = vert.tangent.xyz;
    float tangentW = vert.tangent.w;
    vec2 texCoord = vert.uv;

    // Transform to world space
    vec4 worldPos = push.model * vec4(localPos, 1.0);

    // Wind animation - more effect on higher branches
    ...
    // Branch level affects sway amount (higher = more sway)
    float swayFactor = branchLevel / 3.0;  // Normalize to 0-1 for levels 0-3
