#pragma once

#include "town_generator2/geom/Point.hpp"
#include "town_generator2/geom/GeomUtils.hpp"
#include "town_generator2/utils/MathUtils.hpp"
#include <vector>
#include <functional>
#include <algorithm>
#include <cmath>
#include <limits>

namespace town_generator2 {
namespace geom {

/**
 * Rectangle - Simple bounding rectangle
 */
struct Rectangle {
    double left = 0, top = 0, right = 0, bottom = 0;

    Rectangle() = default;
    Rectangle(double x, double y) : left(x), top(y), right(x), bottom(y) {}

    double width() const { return right - left; }
    double height() const { return bottom - top; }
};

/**
 * Polygon - Core polygon class, faithful port from Haxe TownGeneratorOS
 *
 * Uses shared_ptr<Point> (PointPtr) for reference semantics - when multiple
 * polygons share the same PointPtr, mutations to the underlying Point are
 * visible to all of them. This is fundamental to how the algorithm works.
 *
 * Key semantic differences from value-based polygon:
 * - Copy constructor shares the same PointPtrs (shallow copy)
 * - Use deepCopy() to create independent points
 * - indexOf/contains use pointer identity by default (like Haxe == on references)
 * - Use indexOfByValue/containsByValue for coordinate comparison
 */
class Polygon {
private:
    static constexpr double DELTA = 0.000001;
    std::vector<PointPtr> vertices_;

public:
    // Constructors
    Polygon() = default;

    // Construct from shared points (shares the points - reference semantics)
    explicit Polygon(const std::vector<PointPtr>& vertices) : vertices_(vertices) {}

    // Construct from value points (creates new shared points - no sharing)
    explicit Polygon(const std::vector<Point>& vertices) {
        vertices_.reserve(vertices.size());
        for (const auto& v : vertices) {
            vertices_.push_back(makePoint(v));
        }
    }

    Polygon(std::initializer_list<Point> init) {
        vertices_.reserve(init.size());
        for (const auto& v : init) {
            vertices_.push_back(makePoint(v));
        }
    }

    // Copy constructor - shares the same points (reference semantics like Haxe)
    Polygon(const Polygon& other) : vertices_(other.vertices_) {}

    Polygon& operator=(const Polygon& other) {
        if (this != &other) {
            vertices_ = other.vertices_;
        }
        return *this;
    }

    // Move semantics
    Polygon(Polygon&&) noexcept = default;
    Polygon& operator=(Polygon&&) noexcept = default;

    // Deep copy - creates new independent points
    Polygon deepCopy() const {
        std::vector<PointPtr> newVerts;
        newVerts.reserve(vertices_.size());
        for (const auto& v : vertices_) {
            newVerts.push_back(makePoint(*v));
        }
        return Polygon(newVerts);
    }

    // Shallow copy (same as copy constructor, explicit name)
    Polygon copy() const { return Polygon(vertices_); }

    // Size
    size_t length() const { return vertices_.size(); }
    size_t size() const { return vertices_.size(); }
    bool empty() const { return vertices_.empty(); }

    // Access by value (dereferences pointer)
    Point& operator[](size_t i) { return *vertices_[i]; }
    const Point& operator[](size_t i) const { return *vertices_[i]; }

    // Access to shared pointer directly (for pointer identity operations)
    PointPtr& ptr(size_t i) { return vertices_[i]; }
    const PointPtr& ptr(size_t i) const { return vertices_[i]; }

    // Set all vertices from another polygon (Haxe's polygon.set())
    // Mutates the underlying Point values, doesn't change pointers
    void set(const Polygon& p) {
        for (size_t i = 0; i < p.length() && i < vertices_.size(); ++i) {
            vertices_[i]->set(*p.vertices_[i]);
        }
    }

    // Add points
    void push(const Point& p) { vertices_.push_back(makePoint(p)); }
    void push(PointPtr p) { vertices_.push_back(p); }
    void unshift(const Point& p) { vertices_.insert(vertices_.begin(), makePoint(p)); }

    void insert(size_t index, const Point& p) {
        if (index >= vertices_.size()) {
            vertices_.push_back(makePoint(p));
        } else {
            vertices_.insert(vertices_.begin() + index, makePoint(p));
        }
    }

    void splice(size_t start, size_t deleteCount) {
        if (start < vertices_.size()) {
            auto end = std::min(start + deleteCount, vertices_.size());
            vertices_.erase(vertices_.begin() + start, vertices_.begin() + end);
        }
    }

    // Remove by pointer identity (like Haxe)
    bool remove(const PointPtr& p) {
        auto it = std::find(vertices_.begin(), vertices_.end(), p);
        if (it != vertices_.end()) {
            vertices_.erase(it);
            return true;
        }
        return false;
    }

    // Remove by value (coordinate match)
    bool removeByValue(const Point& p) {
        for (auto it = vertices_.begin(); it != vertices_.end(); ++it) {
            if (**it == p) {
                vertices_.erase(it);
                return true;
            }
        }
        return false;
    }

    Point& last() { return *vertices_.back(); }
    const Point& last() const { return *vertices_.back(); }
    PointPtr lastPtr() const { return vertices_.back(); }

    // Slice returning new polygon with same shared pointers
    Polygon slice(size_t start) const {
        if (start >= vertices_.size()) return Polygon();
        return Polygon(std::vector<PointPtr>(vertices_.begin() + start, vertices_.end()));
    }

    Polygon slice(size_t start, size_t end) const {
        if (start >= vertices_.size()) return Polygon();
        end = std::min(end, vertices_.size());
        return Polygon(std::vector<PointPtr>(vertices_.begin() + start, vertices_.begin() + end));
    }

    // Concatenate
    Polygon concat(const Polygon& other) const {
        std::vector<PointPtr> result = vertices_;
        result.insert(result.end(), other.vertices_.begin(), other.vertices_.end());
        return Polygon(result);
    }

    // Iterator support
    auto begin() { return vertices_.begin(); }
    auto end() { return vertices_.end(); }
    auto begin() const { return vertices_.begin(); }
    auto end() const { return vertices_.end(); }

    // Find by pointer identity (like Haxe == on references)
    int indexOf(const PointPtr& v) const {
        for (size_t i = 0; i < vertices_.size(); ++i) {
            if (vertices_[i] == v) return static_cast<int>(i);
        }
        return -1;
    }

    // Find by value (coordinate match)
    int indexOfByValue(const Point& v) const {
        for (size_t i = 0; i < vertices_.size(); ++i) {
            if (*vertices_[i] == v) return static_cast<int>(i);
        }
        return -1;
    }

    int lastIndexOf(const PointPtr& v) const {
        for (int i = static_cast<int>(vertices_.size()) - 1; i >= 0; --i) {
            if (vertices_[i] == v) return i;
        }
        return -1;
    }

    int lastIndexOfByValue(const Point& v) const {
        for (int i = static_cast<int>(vertices_.size()) - 1; i >= 0; --i) {
            if (*vertices_[i] == v) return i;
        }
        return -1;
    }

    bool contains(const PointPtr& v) const { return indexOf(v) != -1; }
    bool containsByValue(const Point& v) const { return indexOfByValue(v) != -1; }

    // Computed properties

    // Signed area (shoelace formula) - positive for CCW, negative for CW
    double square() const {
        if (vertices_.size() < 3) return 0;

        const Point& v1_init = *vertices_.back();
        const Point& v2_init = *vertices_[0];
        double s = v1_init.x * v2_init.y - v2_init.x * v1_init.y;

        for (size_t i = 1; i < vertices_.size(); ++i) {
            const Point& v1 = *vertices_[i - 1];
            const Point& v2 = *vertices_[i];
            s += (v1.x * v2.y - v2.x * v1.y);
        }
        return s * 0.5;
    }

    double perimeter() const {
        double len = 0.0;
        forEdge([&len](const Point& v0, const Point& v1) {
            len += Point::distance(v0, v1);
        });
        return len;
    }

    // Compactness: circle=1.0, square=0.79, triangle=0.60
    double compactness() const {
        double p = perimeter();
        return 4 * M_PI * square() / (p * p);
    }

    // Simple average of vertices
    Point center() const {
        Point c;
        for (const auto& v : vertices_) {
            c.addEq(*v);
        }
        c.scaleEq(1.0 / vertices_.size());
        return c;
    }

    // Area-weighted centroid
    Point centroid() const {
        double x = 0.0, y = 0.0, a = 0.0;
        forEdge([&x, &y, &a](const Point& v0, const Point& v1) {
            double f = GeomUtils::cross(v0.x, v0.y, v1.x, v1.y);
            a += f;
            x += (v0.x + v1.x) * f;
            y += (v0.y + v1.y) * f;
        });
        double s6 = 1.0 / (3.0 * a);
        return Point(s6 * x, s6 * y);
    }

    // Iterate over edges (closed polygon)
    void forEdge(std::function<void(const Point&, const Point&)> f) const {
        size_t len = vertices_.size();
        for (size_t i = 0; i < len; ++i) {
            f(*vertices_[i], *vertices_[(i + 1) % len]);
        }
    }

    // Iterate over edges with PointPtrs (for when identity matters)
    void forEdgePtr(std::function<void(const PointPtr&, const PointPtr&)> f) const {
        size_t len = vertices_.size();
        for (size_t i = 0; i < len; ++i) {
            f(vertices_[i], vertices_[(i + 1) % len]);
        }
    }

    // Iterate over segments (open - doesn't close back to start)
    void forSegment(std::function<void(const Point&, const Point&)> f) const {
        for (size_t i = 0; i + 1 < vertices_.size(); ++i) {
            f(*vertices_[i], *vertices_[i + 1]);
        }
    }

    // Offset all vertices (mutates shared points!)
    void offset(const Point& p) {
        for (auto& v : vertices_) {
            v->offset(p.x, p.y);
        }
    }

    void offset(double dx, double dy) {
        for (auto& v : vertices_) {
            v->offset(dx, dy);
        }
    }

    // Rotate all vertices around origin (mutates shared points!)
    void rotate(double a) {
        double cosA = std::cos(a);
        double sinA = std::sin(a);
        for (auto& v : vertices_) {
            double vx = v->x * cosA - v->y * sinA;
            double vy = v->y * cosA + v->x * sinA;
            v->setTo(vx, vy);
        }
    }

    // Check if vertex at index is convex
    bool isConvexVertexi(int i) const {
        int len = static_cast<int>(vertices_.size());
        const Point& v0 = *vertices_[(i + len - 1) % len];
        const Point& v1 = *vertices_[i];
        const Point& v2 = *vertices_[(i + 1) % len];
        return GeomUtils::cross(v1.x - v0.x, v1.y - v0.y, v2.x - v1.x, v2.y - v1.y) > 0;
    }

    bool isConvexVertex(const PointPtr& v1) const {
        const Point& v0 = *prev(v1);
        const Point& v2 = *next(v1);
        return GeomUtils::cross(v1->x - v0.x, v1->y - v0.y, v2.x - v1->x, v2.y - v1->y) > 0;
    }

    bool isConvex() const {
        for (size_t i = 0; i < vertices_.size(); ++i) {
            if (!isConvexVertexi(i)) return false;
        }
        return true;
    }

    // Smooth vertex at index
    Point smoothVertexi(int i, double f = 1.0) const {
        const Point& v = *vertices_[i];
        int len = static_cast<int>(vertices_.size());
        const Point& prevV = *vertices_[(i + len - 1) % len];
        const Point& nextV = *vertices_[(i + 1) % len];
        return Point(
            (prevV.x + v.x * f + nextV.x) / (2 + f),
            (prevV.y + v.y * f + nextV.y) / (2 + f)
        );
    }

    Point smoothVertex(const PointPtr& v, double f = 1.0) const {
        const Point& prevV = *prev(v);
        const Point& nextV = *next(v);
        return Point(
            prevV.x + v->x * f + nextV.x,
            prevV.y + v->y * f + nextV.y
        ).scale(1.0 / (2 + f));
    }

    // Smooth all vertices - returns new polygon with new points
    Polygon smoothVertexEq(double f = 1.0) const {
        size_t len = vertices_.size();
        std::vector<Point> result;
        result.reserve(len);

        Point v1 = *vertices_[len - 1];
        Point v2 = *vertices_[0];

        for (size_t i = 0; i < len; ++i) {
            Point v0 = v1;
            v1 = v2;
            v2 = *vertices_[(i + 1) % len];
            result.emplace_back(
                (v0.x + v1.x * f + v2.x) / (2 + f),
                (v0.y + v1.y * f + v2.y) / (2 + f)
            );
        }
        return Polygon(result);
    }

    // Filter out short edges - returns new polygon with new points
    Polygon filterShort(double threshold) const {
        if (vertices_.empty()) return Polygon();

        std::vector<Point> result;
        Point v0 = *vertices_[0];
        result.push_back(v0);

        size_t i = 1;
        while (i < vertices_.size()) {
            Point v1;
            do {
                v1 = *vertices_[i++];
            } while (Point::distance(v0, v1) < threshold && i < vertices_.size());
            result.push_back(v1);
            v0 = v1;
        }

        return Polygon(result);
    }

    // Minimal distance from any vertex to a point
    double distance(const Point& p) const {
        if (vertices_.empty()) return std::numeric_limits<double>::infinity();

        double d = Point::distance(*vertices_[0], p);
        for (size_t i = 1; i < vertices_.size(); ++i) {
            double d1 = Point::distance(*vertices_[i], p);
            if (d1 < d) d = d1;
        }
        return d;
    }

    // Find edge by pointer identity
    int findEdge(const PointPtr& a, const PointPtr& b) const {
        int index = indexOf(a);
        if (index != -1 && vertices_[(index + 1) % vertices_.size()] == b) {
            return index;
        }
        return -1;
    }

    // Find edge by value
    int findEdgeByValue(const Point& a, const Point& b) const {
        int index = indexOfByValue(a);
        if (index != -1 && *vertices_[(index + 1) % vertices_.size()] == b) {
            return index;
        }
        return -1;
    }

    // Get next/prev by pointer identity
    PointPtr next(const PointPtr& a) const {
        return vertices_[(indexOf(a) + 1) % vertices_.size()];
    }

    PointPtr prev(const PointPtr& a) const {
        int idx = indexOf(a);
        return vertices_[(idx + vertices_.size() - 1) % vertices_.size()];
    }

    // Get next/prev by index
    PointPtr nexti(int i) const {
        return vertices_[(i + 1) % vertices_.size()];
    }

    PointPtr previ(int i) const {
        return vertices_[(i + vertices_.size() - 1) % vertices_.size()];
    }

    // Vector from vertex to next vertex
    Point vector(const PointPtr& v) const {
        return next(v)->subtract(*v);
    }

    Point vectori(int i) const {
        int nextIdx = (i == static_cast<int>(vertices_.size()) - 1) ? 0 : i + 1;
        return vertices_[nextIdx]->subtract(*vertices_[i]);
    }

    // Check if this polygon borders another (shares an edge)
    bool borders(const Polygon& another) const {
        size_t len1 = vertices_.size();
        size_t len2 = another.length();

        for (size_t i = 0; i < len1; ++i) {
            // Use pointer identity to find shared vertices
            int j = another.indexOf(vertices_[i]);
            if (j != -1) {
                const PointPtr& nextP = vertices_[(i + 1) % len1];
                if (nextP == another.ptr((j + 1) % len2) ||
                    nextP == another.ptr((j + len2 - 1) % len2)) {
                    return true;
                }
            }
        }
        return false;
    }

    // Get bounding rectangle
    Rectangle getBounds() const {
        if (vertices_.empty()) return Rectangle();

        Rectangle rect(vertices_[0]->x, vertices_[0]->y);
        for (const auto& v : vertices_) {
            rect.left = std::min(rect.left, v->x);
            rect.right = std::max(rect.right, v->x);
            rect.top = std::min(rect.top, v->y);
            rect.bottom = std::max(rect.bottom, v->y);
        }
        return rect;
    }

    // Split polygon at two vertices
    std::vector<Polygon> split(const PointPtr& p1, const PointPtr& p2) const {
        return spliti(indexOf(p1), indexOf(p2));
    }

    std::vector<Polygon> spliti(int i1, int i2) const {
        if (i1 > i2) std::swap(i1, i2);

        std::vector<Polygon> result;
        result.push_back(slice(i1, i2 + 1));
        result.push_back(slice(i2).concat(slice(0, i1 + 1)));

        return result;
    }

    // Cut polygon with a line - returns two halves
    std::vector<Polygon> cut(const Point& p1, const Point& p2, double gap = 0.0) const {
        double x1 = p1.x;
        double y1 = p1.y;
        double dx1 = p2.x - x1;
        double dy1 = p2.y - y1;

        size_t len = vertices_.size();
        int edge1 = 0, edge2 = 0;
        double ratio1 = 0.0, ratio2 = 0.0;
        int count = 0;

        for (size_t i = 0; i < len; ++i) {
            const Point& v0 = *vertices_[i];
            const Point& v1 = *vertices_[(i + 1) % len];

            double x2 = v0.x;
            double y2 = v0.y;
            double dx2 = v1.x - x2;
            double dy2 = v1.y - y2;

            auto t = GeomUtils::intersectLines(x1, y1, dx1, dy1, x2, y2, dx2, dy2);
            if (t && t->y >= 0 && t->y <= 1) {
                if (count == 0) {
                    edge1 = static_cast<int>(i);
                    ratio1 = t->x;
                } else if (count == 1) {
                    edge2 = static_cast<int>(i);
                    ratio2 = t->x;
                }
                count++;
            }
        }

        if (count == 2) {
            Point diff = p2.subtract(p1);
            Point point1 = p1.add(diff.scale(ratio1));
            Point point2 = p1.add(diff.scale(ratio2));

            Polygon half1 = slice(edge1 + 1, edge2 + 1);
            half1.unshift(point1);
            half1.push(point2);

            Polygon half2 = slice(edge2 + 1).concat(slice(0, edge1 + 1));
            half2.unshift(point2);
            half2.push(point1);

            if (gap > 0) {
                half1 = half1.peel(half1.ptr(half1.length() - 2), gap / 2);
                half2 = half2.peel(half2.ptr(half2.length() - 2), gap / 2);
            }

            Point v = vectori(edge1);
            return GeomUtils::cross(dx1, dy1, v.x, v.y) > 0
                ? std::vector<Polygon>{half1, half2}
                : std::vector<Polygon>{half2, half1};
        } else {
            return {deepCopy()};
        }
    }

    // Inset one edge (mutates polygon)
    void inset(const PointPtr& p1, double d) {
        int i1 = indexOf(p1);
        int i0 = (i1 > 0 ? i1 - 1 : static_cast<int>(vertices_.size()) - 1);
        int i2 = (i1 < static_cast<int>(vertices_.size()) - 1 ? i1 + 1 : 0);
        int i3 = (i2 < static_cast<int>(vertices_.size()) - 1 ? i2 + 1 : 0);

        const Point& p0 = *vertices_[i0];
        const Point& p2 = *vertices_[i2];
        const Point& p3 = *vertices_[i3];

        Point v0 = p1->subtract(p0);
        Point v1 = p2.subtract(*p1);
        Point v2 = p3.subtract(p2);

        double cos_val = v0.dot(v1) / v0.length() / v1.length();
        double z = v0.x * v1.y - v0.y * v1.x;
        double t = d / std::sqrt(1 - cos_val * cos_val);
        if (z > 0) {
            t = std::min(t, v0.length() * 0.99);
        } else {
            t = std::min(t, v1.length() * 0.5);
        }
        t *= utils::MathUtils::sign(z);
        vertices_[i1] = makePoint(p1->subtract(v0.norm(t)));

        cos_val = v1.dot(v2) / v1.length() / v2.length();
        z = v1.x * v2.y - v1.y * v2.x;
        t = d / std::sqrt(1 - cos_val * cos_val);
        if (z > 0) {
            t = std::min(t, v2.length() * 0.99);
        } else {
            t = std::min(t, v1.length() * 0.5);
        }
        vertices_[i2] = makePoint(p2.add(v2.norm(t)));
    }

    // Inset all edges
    void insetEq(double d) {
        for (size_t i = 0; i < vertices_.size(); ++i) {
            inset(vertices_[i], d);
        }
    }

    // Buffer (offset) all edges - returns new polygon
    Polygon buffer(const std::vector<double>& d) const {
        // Create polygon with offset edges
        Polygon q;
        size_t i = 0;
        forEdge([&q, &d, &i](const Point& v0, const Point& v1) {
            double dd = d[i++];
            if (dd == 0) {
                q.push(v0);
                q.push(v1);
            } else {
                Point v = v1.subtract(v0);
                Point n = v.rotate90().norm(dd);
                q.push(v0.add(n));
                q.push(v1.add(n));
            }
        });

        // Handle self-intersection
        bool wasCut;
        size_t lastEdge = 0;
        do {
            wasCut = false;
            size_t n = q.length();

            for (size_t ii = lastEdge; ii + 2 < n; ++ii) {
                lastEdge = ii;

                const Point& p11 = q[ii];
                const Point& p12 = q[ii + 1];
                double x1 = p11.x;
                double y1 = p11.y;
                double dx1 = p12.x - x1;
                double dy1 = p12.y - y1;

                size_t jEnd = (ii > 0) ? n : n - 1;
                for (size_t jj = ii + 2; jj < jEnd; ++jj) {
                    const Point& p21 = q[jj];
                    const Point& p22 = (jj < n - 1) ? q[jj + 1] : q[0];
                    double x2 = p21.x;
                    double y2 = p21.y;
                    double dx2 = p22.x - x2;
                    double dy2 = p22.y - y2;

                    auto intPt = GeomUtils::intersectLines(x1, y1, dx1, dy1, x2, y2, dx2, dy2);
                    if (intPt && intPt->x > DELTA && intPt->x < 1 - DELTA &&
                        intPt->y > DELTA && intPt->y < 1 - DELTA) {
                        Point pn(x1 + dx1 * intPt->x, y1 + dy1 * intPt->x);

                        q.insert(jj + 1, pn);
                        q.insert(ii + 1, pn);

                        wasCut = true;
                        break;
                    }
                }
                if (wasCut) break;
            }
        } while (wasCut);

        // Find largest part
        std::vector<int> regular;
        for (size_t ii = 0; ii < q.length(); ++ii) {
            regular.push_back(static_cast<int>(ii));
        }

        Polygon bestPart;
        double bestPartSq = -std::numeric_limits<double>::infinity();

        while (!regular.empty()) {
            std::vector<int> indices;
            int start = regular[0];
            int idx = start;

            do {
                indices.push_back(idx);
                regular.erase(std::find(regular.begin(), regular.end(), idx));

                int nextIdx = (idx + 1) % static_cast<int>(q.length());
                const PointPtr& v = q.ptr(nextIdx);
                int next1 = q.indexOf(v);
                if (next1 == nextIdx) {
                    next1 = q.lastIndexOf(v);
                }
                idx = (next1 == -1) ? nextIdx : next1;
            } while (idx != start);

            std::vector<PointPtr> partVerts;
            for (int pi : indices) {
                partVerts.push_back(q.ptr(pi));
            }
            Polygon p(partVerts);
            double s = p.square();
            if (s > bestPartSq) {
                bestPart = p.deepCopy();
                bestPartSq = s;
            }
        }

        return bestPart;
    }

    Polygon bufferEq(double d) const {
        std::vector<double> distances(vertices_.size(), d);
        return buffer(distances);
    }

    // Shrink polygon by cutting with offset lines
    Polygon shrink(const std::vector<double>& d) const {
        Polygon q = deepCopy();
        size_t i = 0;
        forEdge([&q, &d, &i](const Point& v1, const Point& v2) {
            double dd = d[i++];
            if (dd > 0) {
                Point v = v2.subtract(v1);
                Point n = v.rotate90().norm(dd);
                auto parts = q.cut(v1.add(n), v2.add(n), 0);
                if (!parts.empty()) {
                    q = parts[0];
                }
            }
        });
        return q;
    }

    Polygon shrinkEq(double d) const {
        std::vector<double> distances(vertices_.size(), d);
        return shrink(distances);
    }

    // Peel one edge
    Polygon peel(const PointPtr& v1, double d) const {
        int i1 = indexOf(v1);
        int i2 = (i1 == static_cast<int>(vertices_.size()) - 1) ? 0 : i1 + 1;
        const Point& v2 = *vertices_[i2];

        Point v = v2.subtract(*v1);
        Point n = v.rotate90().norm(d);

        auto parts = cut(v1->add(n), v2.add(n), 0);
        return parts.empty() ? Polygon() : parts[0];
    }

    // Simplify polygon to n vertices
    void simplify(int n) {
        int len = static_cast<int>(vertices_.size());
        while (len > n) {
            int result = 0;
            double minVal = std::numeric_limits<double>::infinity();

            const Point& b_init = *vertices_[len - 1];
            const Point& c_init = *vertices_[0];
            Point b = b_init;
            Point c = c_init;

            for (int i = 0; i < len; ++i) {
                Point a = b;
                b = c;
                c = *vertices_[(i + 1) % len];
                double measure = std::abs(a.x * (b.y - c.y) + b.x * (c.y - a.y) + c.x * (a.y - b.y));
                if (measure < minVal) {
                    result = i;
                    minVal = measure;
                }
            }

            splice(result, 1);
            len--;
        }
    }

    // Interpolation weights for a point
    std::vector<double> interpolate(const Point& p) const {
        double sum = 0.0;
        std::vector<double> dd;
        dd.reserve(vertices_.size());

        for (const auto& v : vertices_) {
            double d = 1.0 / Point::distance(*v, p);
            sum += d;
            dd.push_back(d);
        }

        for (auto& di : dd) {
            di /= sum;
        }
        return dd;
    }

    // Filter vertices
    Polygon filter(std::function<bool(const Point&)> pred) const {
        std::vector<PointPtr> result;
        for (const auto& v : vertices_) {
            if (pred(*v)) result.push_back(v);
        }
        return Polygon(result);
    }

    // Find min/max by function
    template<typename F>
    PointPtr min(F f) const {
        size_t resultIdx = 0;
        double minVal = f(*vertices_[0]);
        for (size_t i = 1; i < vertices_.size(); ++i) {
            double val = f(*vertices_[i]);
            if (val < minVal) {
                resultIdx = i;
                minVal = val;
            }
        }
        return vertices_[resultIdx];
    }

    template<typename F>
    PointPtr max(F f) const {
        size_t resultIdx = 0;
        double maxVal = f(*vertices_[0]);
        for (size_t i = 1; i < vertices_.size(); ++i) {
            double val = f(*vertices_[i]);
            if (val > maxVal) {
                resultIdx = i;
                maxVal = val;
            }
        }
        return vertices_[resultIdx];
    }

    // Count vertices matching predicate
    int count(std::function<bool(const Point&)> test) const {
        int cnt = 0;
        for (const auto& v : vertices_) {
            if (test(*v)) cnt++;
        }
        return cnt;
    }

    // Static factory methods
    static Polygon rect(double w = 1.0, double h = 1.0) {
        return Polygon({
            Point(-w / 2, -h / 2),
            Point(w / 2, -h / 2),
            Point(w / 2, h / 2),
            Point(-w / 2, h / 2)
        });
    }

    static Polygon regular(int n = 8, double r = 1.0) {
        std::vector<Point> points;
        points.reserve(n);
        for (int i = 0; i < n; ++i) {
            double a = static_cast<double>(i) / n * M_PI * 2;
            points.emplace_back(r * std::cos(a), r * std::sin(a));
        }
        return Polygon(points);
    }

    static Polygon circle(double r = 1.0) {
        return regular(16, r);
    }

    // Access underlying vector
    std::vector<PointPtr>& vertices() { return vertices_; }
    const std::vector<PointPtr>& vertices() const { return vertices_; }
};

} // namespace geom
} // namespace town_generator2
